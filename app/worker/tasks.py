    ...
    try:
        ...
        for source in dicomweb_sources:
            ...
            try:
                ...
                log.info(f"Checked DICOMWeb source {source.id} ({source.source_name}): {health_status.value}")
                
            except Exception as e:
                log.error(f"Error checking DICOMWeb source {source.id}: {str(e)}")
                results["dicomweb_sources"]["error"] += 1
        ...
        google_healthcare_sources = crud.google_healthcare_source.get_multi(db, limit=1000)
        ...
        total_checked = sum(source_type["checked"] for source_type in results.values())
        total_ok = sum(source_type["ok"] for source_type in results.values())
        total_down = sum(source_type["down"] for source_type in results.values())
        total_error = sum(source_type["error"] for source_type in results.values())
        ...
        log.error("Error during health monitoring task", error=str(e), exc_info=True)
        if db and db.is_active:
            db.rollback()
        raise
    finally:
