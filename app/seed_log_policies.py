...
def seed_default_policies():
    ...
    try:
        ...
        archival_rule = models.LogArchivalRule(
            name="Critical Logs to GCS Archive",
            description="Archive critical DICOM logs older than 1 year to GCS cold storage",
            service_pattern="axiom.dcm4che-*,axiom.storescp-*",
            age_threshold_days=365,
            storage_backend="gcs",
            storage_bucket="axiom-log-archive",
            storage_path_prefix="critical-logs/",
            compression="gzip",
            format_type="ndjson",
            retention_days=2555,  # 7 years in archive
            delete_after_archive=True,
            cron_schedule="0 2 * * 0"  # Weekly on Sundays at 2 AM
        )
